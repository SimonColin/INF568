...
/*void ladder(mpz_t N, mpz_t a, mpz_t m, mpz_t u, mpz_t Xm, mpz_t Zm)
{
	int i, n, swap;
	mpz_t a24, x1, x2, z2, x3, z3, A, AA, B, BB, E, C, D, DA, CB;
	mp_limb_t *limbs_x2, *limbs_x3, *limbs_z2, *limbs_z3;
	mpz_inits(a24, x1, x2, z2, x3, z3, A, AA, B, BB, E, C, D, DA, CB, NULL);
	size = mpz_size(N);
	calc_A(a24, a, N);
//	gmp_printf("a24 = %Zd\n", a24);
	mpz_set(x1, u);
...
//	n = mod2(m);
	...
	while(i >= 0)
	{
		printf("n = %x\nswap = %x\n", n, swap);
		swap ^= n;
		limbs_x2 = mpz_limbs_modify(x2, size);
		limbs_z2 = mpz_limbs_modify(z2, size);
		limbs_x3 = mpz_limbs_modify(x3, size);
		limbs_z3 = mpz_limbs_modify(z3, size);
		mpz_limbs_finish(x2, size);
		mpz_limbs_finish(z2, size);
		mpz_limbs_finish(x3, size);
		mpz_limbs_finish(z3, size);
		gmp_printf("x1 = %Zd\nz1 = %Zd\n", x2, z2);
		
		cswap(swap, limbs_x2, limbs_x3, size);
		cswap(swap, limbs_z2, limbs_z3, size);
		gmp_printf("x1 = %Zd\nz1 = %Zd\n", x2, z2);
	...
	mpz_clears(a24, x1, x2, z2, x3, z3, A, AA, B, BB, E, C, D, DA, CB, NULL);
}*/
